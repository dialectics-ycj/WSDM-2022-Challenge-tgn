...
class LastMessageAggregator(MessageAggregator):
  ...
  def aggregate(self, node_ids, messages):
    """Only keep the last message for each node who has really message in messages"""
    unique_node_ids = np.unique(node_ids)
    # aggregate后的node-message-timestamp，三个对应数组
    to_update_node_ids = []
    unique_messages = []
    unique_timestamps = []
    ...
    for node_id in unique_node_ids:
        if len(messages[node_id]) > 0:  # 只有那些有message的节点才能保留下来
            to_update_node_ids.append(node_id)
            unique_messages.append(messages[node_id][-1][0])
            unique_timestamps.append(messages[node_id][-1][1])
    
    unique_messages = torch.stack(unique_messages) if len(to_update_node_ids) > 0 else []   # tensor list -> tensor
...
class MeanMessageAggregator(MessageAggregator):
  ...
  def aggregate(self, node_ids, messages):
    """Compute the mean message for each node who has really message in messages"""
    unique_node_ids = np.unique(node_ids)
    unique_messages = []
    unique_timestamps = []
    ...
    for node_id in unique_node_ids:
      if len(messages[node_id]) > 0:
        ...
        unique_timestamps.append(messages[node_id][-1][1])  # 只保留最新时间
