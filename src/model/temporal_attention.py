...
class TemporalAttentionLayer(torch.nn.Module):
    ...
    def __init__(self, node_hidden_emb_dim, n_edge_features, time_dim,
        ...
        self.query_dim = node_hidden_emb_dim + time_dim
        self.key_dim = node_hidden_emb_dim + time_dim + n_edge_features

        self.merger = MergeLayer(self.query_dim, node_hidden_emb_dim, output_dimension, output_dimension)
    ...
    def forward(self, src_node_features, src_time_features, neighbors_features,
        ...
        key = torch.cat([neighbors_features, neighbors_time_features, edge_features], dim=2)
        ...
        attn_output = attn_output.squeeze(dim=0)  # seq, batch, feats
        # attn_output_weights = attn_output_weights.squeeze(dim=0)
...
class TemporalAttentionLayer_EX(torch.nn.Module):
    """
    Temporal attention layer. Return the temporal embedding of a node given the node itself,
     its neighbors and the edge timestamps.
    """
    ...
    def __init__(self, node_hidden_emb_dim, n_edge_features, time_dim,
                 output_dimension, n_head=2,
                 dropout=0.1):
        super(TemporalAttentionLayer_EX, self).__init__()
        ...
        self.merger = MergeLayer(self.query_dim, self.query_dim, output_dimension, output_dimension)
    ...
    def forward(self, src_node_features, src_time_features, neighbors_features,
        ...
        src_node_features_unrolled = torch.unsqueeze(src_node_features, dim=0)
        src_time_features = src_time_features.permute([1, 0, 2])
        query = torch.cat([src_node_features_unrolled, src_time_features], dim=2)
        key = torch.cat([neighbors_features, neighbors_time_features, edge_features], dim=2)
        ...
        # Skip connection with temporal attention over neighborhood and the features of the node itself
        attn_output = self.merger(query.squeeze(dim=0), attn_output)
